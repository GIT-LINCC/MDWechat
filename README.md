...
2. 新增 MD3 风格主界面，覆盖底部 Tab、悬浮按钮、会话列表、通讯录、发现、我等页面。
3. 新增 MD3 风格聊天气泡，支持连续消息智能分组、圆角联动、阴影层次、头像/昵称对齐、引用消息和自定义颜色。
4. 扩展卡片类消息适配，覆盖图片、视频、语音、通话、红包、转账、网页链接、小程序、位置、个人名片等常见消息。
5. 简要改善进入会话和滚动时的显示稳定性；构建环境升级到 `AGP 7.1.2 + Gradle 7.5 + Kotlin 1.6.21`。
...
16. 常见聊天卡片 MD3 化，包含图片、视频、语音、通话、红包、转账、网页链接、小程序、位置、个人名片等消息。
17. ~~发现页面支持隐藏朋友圈/扫一扫/摇一摇/附近的人/游戏/购物/小程序~~(微信自带,2.0已去掉)
18. ~~移除会话列表下拉小程序,最低支持微信 6.6.2~~(微信7.0.0以上失效)
19. 识别微X模块入口,移动到悬浮按钮(2.0新增)
20. 主界面字体颜色修改(2.0新增)
